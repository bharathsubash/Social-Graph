...
public class NotifyingBlockingThreadPoolExecutor extends ThreadPoolExecutor {
	...
	private Set<Runnable> pendingTask = Collections.synchronizedSet(new HashSet<Runnable>());
	...
	public NotifyingBlockingThreadPoolExecutor(int poolSize, int queueSize, long keepAliveTime,
		...
		super(poolSize, // Core size
				poolSize, // Max size
				keepAliveTime, keepAliveTimeUnit, new ArrayBlockingQueue<Runnable>(Math.max(poolSize, queueSize)), // not
				// smaller
				// than
				// the
				// poolSize
				...
				// redundant
				// threads)
				new BlockThenRunPolicy(maxBlockingTime, maxBlockingTimeUnit, blockingTimeCallback)); // When
		// super
		// invokes
		// the
		// reject
		...
		super.allowCoreThreadTimeOut(false); // Time out the core threads
	}
	...
	public Set<Runnable> getPendingTask() {
		Set<Runnable> outSet = new HashSet<Runnable>();
		outSet.addAll(pendingTask);
		return outSet;
	}
	 ...
	 * Java 6 source code).
	 */
	public static class BlockThenRunPolicy implements RejectedExecutionHandler {
